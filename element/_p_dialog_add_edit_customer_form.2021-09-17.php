...
?>
						...
						<div class="form-row justify-content-center text-center">	
							...
							<div class="col-md-6">
								<div class="form-group">
									<label class="form-control-label mb-0">Default Sender name</label>
									<input class="form-control text-center" name="CUSTOMER_Sender" id="CUSTOMER_Sender" value="<?=$acc['CUSTOMER_Sender']?>" />
								</div>
							</div>
						</div>
						<?php
						if (strlen($user_id) && $_SESSION[_SES_USR_TYP_ID]<10) {
						?>
						<div class="form-row justify-content-center text-center">	
							<div class="col-md-12">
								<div class="form-group mb-0">
									<label class="form-control-label mb-0">Multiple Sender Name (up to 9 digits a-Z,0-9 and @ ! # _ - . )</label>
								</div>
							</div>
						</div>
						<div class="form-row justify-content-center text-center">
							<?php
							$stmt=querySqlEx("SELECT SENDER_Sender FROM SBG.dbo.SENDERS WHERE SENDER_Username=?",[$user_id]);
							while ($row=$stmt->fetch(PDO::FETCH_ASSOC)) $exist_senders[]=$row['SENDER_Sender'];
							for($i=0;$i<12;$i++) {
							?>
							<div class="col-md-3">
								<div class="form-group">
									<input class="form-control" name="CUSTOMER_Senders[]" id="CUSTOMER_Senders[]" value="<?=(isset($exist_senders[$i])?$exist_senders[$i]:'')?>" placeholder="Sender <?=($i+1)?>" />
								</div>
							</div>
							<?php
							}
