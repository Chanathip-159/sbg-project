...
$('#EditChangeDelModal').on('show.bs.modal', function(e) {
	...
	form_acc_edit_acc.action = "<?=_MAIN_DF;?>keyword";
	if (user_id.substr(0, 4) == "del-") {
		...
		document.getElementById("add-edit-sender-modal-header").innerHTML="Delete keyword : "+user_id.split("-")[1];
		...
		document.getElementById("add-edit-sender-modal-header").innerHTML="Edit keyword : "+user_id;
	...
	if (load_sub) {
		$.ajax({
			cache: false,
			type: 'POST',
			url: 'element/_p_dialog_add_edit_keyword_form.php',
