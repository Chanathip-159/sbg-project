...
function sendSMSEngine($user,$hpass,$from,$target,$mess,$lang,$expire=null,$scheduled=null)  {
	...
	foreach ($users as $user) {
		...
		// check user
		$sql="SELECT GETDATE() AS 'current_datetime',CUSTOMER_Password, CUSTOMER_Sender,CUSTOMER_ActivateDate,CUSTOMER_ExpireDate,CUSTOMER_MonthlyUsage,CUSTOMER_AccountUsage,CUSTOMER_CurrentUsage,CUSTOMER_NeedDr,CUSTOMER_Status
		FROM SBG.dbo.CUSTOMERS
		WHERE CUSTOMER_Username=?";

		$account=querySqlSingleRowEx($sql,[$user]);
		if (!$account['CUSTOMER_ActivateDate']) {
			$return=Array('error_code'=>8,'error'=>"invalid username ($user).");
			writeLog($log_msg,$return);
			sleep(_PAUSE);
			return $return;
		}
		// check password
		// if ($account['CUSTOMER_Password'] == $hpass) {
		// 	$return=Array('error_code'=>3,'error'=>"invalid password ($hpass).");
		// 	writeLog($log_msg,$return);
		// 	sleep(_PAUSE);
		// 	return $return;
		// }
		...
		if (isset($account['CUSTOMER_ActivateDate'])&&strlen($account['CUSTOMER_ActivateDate'])&&time()<strtotime($account['CUSTOMER_ActivateDate'])) {
			$return=Array('error_code'=>9,'error'=>"account $user level $level idle.");
			writeLog($log_msg,$return);
			sleep(_PAUSE);
			return $return;
		}
		...
		if (isset($account['CUSTOMER_ExpireDate'])&&strlen($account['CUSTOMER_ExpireDate'])&&time()>strtotime($account['CUSTOMER_ExpireDate'])) {
			$return=Array('error_code'=>10,'error'=>"account $user level $level expired.");
			writeLog($log_msg,$return);
			sleep(_PAUSE);
			return $return;
		}
		...
		if (($account['CUSTOMER_CurrentUsage']+$total_target>$account['CUSTOMER_AccountUsage'])&&($account['CUSTOMER_AccountUsage']!="-1")) {
			$return=Array('error_code'=>11,'error'=>"account $user level $level out of credit (a). C:" . $account['CUSTOMER_CurrentUsage'] . ",T:" . $total_target . ",A:" . $account['CUSTOMER_AccountUsage']);
			writeLog($log_msg,$return);
			sleep(_PAUSE);
			return $return;
		}
		// Status
		if ($account['CUSTOMER_Status']!=1) {
			$return=Array('error_code'=>12,'error'=>"account $user level $level not active.");
			writeLog($log_msg,$return);
			sleep(_PAUSE);
			return $return;
		}
	...
	if ($from!=$account['CUSTOMER_Sender']) {
		//if (querySqlSingleFieldEx("SELECT COUNT(*) count_sender FROM SBG.dbo.SENDERS WHERE SENDER_Username=?",[$user])) {
			...
			$match=false;
			$active=false;
			$stmt=querySqlEx("SELECT SENDER_Sender,SENDER_Status FROM SBG.dbo.SENDERS WHERE SENDER_Username=? and SENDER_Sender=?",[$user,$from]);
			$log_sender="";
			while ($row=$stmt->fetch(PDO::FETCH_ASSOC)) {
				$log_sender.=$row['SENDER_Sender'].",";
				if ($row['SENDER_Sender']==$from) {
					$match=true;
					if ($row['SENDER_Status']==1) {
						$active=true;
					}
				}
			}
			if (!$match) {
				$return=Array('error_code'=>17,'error'=>"sender [$from] not allow.");
				writeLog($log_msg,$return);
				sleep(_PAUSE);
				return $return;
			}
			else if (!$active) {
				$return=Array('error_code'=>17,'error'=>"sender [$from] not active.");
				writeLog($log_msg,$return);
				sleep(_PAUSE);
				return $return;
			}
	...
	foreach ($users as $user) {
		...
		$sql="SELECT GETDATE() AS 'current_datetime',CUSTOMER_ActivateDate,CUSTOMER_ExpireDate,CUSTOMER_MonthlyUsage,CUSTOMER_AccountUsage,CUSTOMER_CurrentUsage,CUSTOMER_NeedDr,CUSTOMER_Status FROM SBG.dbo.CUSTOMERS WHERE CUSTOMER_Username=?";
		...
		if (!$report['REPORT_Username']) {
			...
			if (($total_target>$account['CUSTOMER_MonthlyUsage'])&&($account['CUSTOMER_MonthlyUsage']!="-1")) {
				$return=Array('error_code'=>12,'error'=>"account $user level $level out of credit (m). T:" . $total_target . ",M:" . $account['CUSTOMER_MonthlyUsage']);
				writeLog($log_msg,$return);
				sleep(_PAUSE);
				return $return;
			}
			...
			if (($report['REPORT_MonthUsage']+$total_target>$report['REPORT_MonthQuota'])&&($report['REPORT_MonthQuota']!="-1")) {
				$return=Array('error_code'=>13,'error'=>"account $user level $level out of credit (m).");
				writeLog($log_msg,$return);
				sleep(_PAUSE);
				return $return;
			}
	...
	$send_data['sms_username_sub']=$current_user;
...
UPDATE SBG.dbo.CUSTOMERS SET CUSTOMER_CurrentUsage=CUSTOMER_CurrentUsage+CONVERT(int,?) WHERE CUSTOMER_Username IN (?,?)";
	querySqlEx($sql,[$total_success,$user,$year_month,$total_success,$user,$current_user]);
