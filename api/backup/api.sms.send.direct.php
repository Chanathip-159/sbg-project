...
$send_data['sms_message'] = mssql_escape($_mess);
...
function SendSMSDirect($data, $dbName = "SMS") {
	...
	if (strlen($data["sms_validity_period"])) {
		...
		if ($time < 0) {
			...
			$days = sprintf("%02d", floor($time / 86400));
			$hours = sprintf("%02d", floor($time / 3600) % 24);
			$minutes = sprintf("%02d", floor($time / 60) % 60);
			$seconds = sprintf("%02d", $time % 60);
			$sms_validity_period = "'0000".$days.$hours.$minutes.$seconds."000R'";
		}
		//*/
	}
...
#		sleep(99999);
	...
	if ($data) {
		...
		for ($current_message=1; $current_message<=$summary_message; $current_message++) {
			...
			$sql="INSERT INTO [$dbName].[dbo].[SBG_pend]
